...
interface BranchFormProps {
  ...
  defaultType?: 'BRANCH' | 'STORE';
  onSave: (branch: Partial<Branch>) => void;
  onCancel: () => void;
  user: any; // Add user to props
}

const BranchForm: React.FC<BranchFormProps> = ({ xarumo, branches, editingBranch, defaultType = 'BRANCH', onSave, onCancel, user }) => {
  ...
  useEffect(() => {
    if (editingBranch) {
      setFormData({
        ...editingBranch,
        type: editingBranch.type || 'BRANCH',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (formData.type === 'BRANCH' && !formData.xarunId) {
      alert("CILAD: Fadlan marka hore abuur Shirkad.");
      return;
    }
    if (formData.type === 'STORE' && !formData.parentBranchId) {
      alert("CILAD: Fadlan dooro Xarunta (Branch) uu bakhaarkaan hoos yimaado.");
      return;
    }
    if (!formData.name?.trim()) {
      alert("Fadlan qor magaca xarunta/bakhaarka.");
      return;
    }
    onSave(formData);
  };
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[30000] flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-4xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300 flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-black text-slate-800 tracking-tight">
                {editingBranch 
                  ? (formData.type === 'STORE' ? 'Bedel Bakhaarka' : 'Bedel Xarunta') 
                  : (formData.type === 'STORE' ? 'Bakhaar Cusub' : 'Xarun Cusub')}
              </h2>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8 space-y-8 no-scrollbar">
          <div className={`grid grid-cols-1 ${formData.type === 'STORE' ? 'md:grid-cols-2' : ''} gap-8`}>
            <div className="space-y-6">
              <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest px-1">Xogta Guud</h3>
              <div className="space-y-4">
                {formData.type === 'BRANCH' && user.role === UserRole.SUPER_ADMIN && (
                  <div>
                ...
                {formData.type === 'STORE' && (
                  <div>
                    <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 px-1">Xarunta (Branch)</label>
                    <select 
                      required
                      className="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-indigo-500 focus:bg-white outline-none font-bold text-slate-800 transition-all"
                      ...
                      onChange={e => {
                        ...
                        setFormData({
                          ...formData, 
                          parentBranchId: selectedBranchId,
                          xarunId: selectedBranch ? selectedBranch.xarunId : formData.xarunId
                        });
                      }}
                    >
                      <option value="">Dooro Xarun...</option>
                      {branches.filter(b => b.type === 'BRANCH').map(b => (
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 px-1">
                    {formData.type === 'STORE' ? 'Magaca Bakhaarka' : 'Magaca Xarunta'}
                  </label>
                  <input 
                    required
                    type="text" 
                    className="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-indigo-500 focus:bg-white outline-none font-bold text-slate-800 transition-all"
                    placeholder={formData.type === 'STORE' ? 'e.g. Bakhaarka Weyn' : 'e.g. Xarunta Bakaaro'}
                    value={formData.name}
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 px-1">Degmada</label>
                    <input 
                      required
                      type="text" 
                      className="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-indigo-500 focus:bg-white outline-none font-bold text-slate-800 transition-all"
          ...
          <div className="pt-8 border-t border-slate-100 flex gap-4 sticky bottom-0 bg-white/90 backdrop-blur-sm pb-2">
             ...
             >
               {editingBranch 
                 ? (formData.type === 'STORE' ? 'Cusboonaysii Bakhaarka' : 'Cusboonaysii Xarunta') 
                 : (formData.type === 'STORE' ? 'Keydi Bakhaarka' : 'Keydi Xarunta')}
             </button>
          </div>
        </form>
      </div>
